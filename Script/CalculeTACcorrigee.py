...
def TAC(DonneesEntree, temps, Ainit, TphysZr, TphysLu):
    ...
    for idx, i in enumerate(organes):
        ...
        AmaxLu=y_corrLuOrgane[len(y_act)-1]
        PartieExtrapolee = Amax*np.exp(-(tempsExt-temps[len(temps)-1])*math.log(2)/TphysZr)
        PartieExtrapoleeLuOrgane = AmaxLu*np.exp(-(tempsExt-temps[len(temps)-1])*math.log(2)/TphysLu)
        
        if i in organes_filtres:
            #print ("LAAAAAAAAA",index, idx, i)
            axs[index].set_title(i)
            axs[index].plot(tempsExt, PartieExtrapoleeLuOrgane, label=i, linewidth=0.7)
            #axs[idx].scatter(x, y_corr, label=i, linewidth=0.7)
            #axs[idx].plot(tempsExt, PartieExtrapolee, label=i, linewidth=0.7)
            #print(x, y_pourc)
            axs[index].scatter(temps, y_corrLuOrgane, label=i, linewidth=0.7)
            axs[index].set_title(i)
            axs[index].set_xlabel('Temps en unité', fontsize=15)
            axs[index].set_ylabel('Activité en unité', fontsize=15)
            index = index+1
    ...
    return ActInt_corr, ActExt_corr, ActExt_corrLu, ActInt_corrLu, PartieExtrapoleeLuOrgane, y_corr_matrice
